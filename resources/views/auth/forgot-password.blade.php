...
<head>
    ...
    <title>Lupa Password - {{ \App\Models\Setting::get('web_name', 'FutsalHub') }}</title>
    
    <!-- Open Graph / Facebook / WhatsApp -->
    <meta property="og:type" content="website">
    <meta property="og:title" content="Lupa Password - {{ \App\Models\Setting::get('web_name', 'FutsalHub') }}">
...
<body class="bg-slate-50 font-sans">
    <div class="min-h-screen w-full flex items-center justify-center p-4 relative overflow-x-hidden bg-slate-50">
        ...
        <!-- Right Side: Request Reset Password Card -->
        <div class="md:col-span-6">
            <div class="white-panel rounded-3xl p-8 shadow-xl text-slate-800">
                <div class="mb-6">
                    <h2 class="text-2xl font-extrabold text-slate-900">Lupa Sandi?</h2>
                    <p class="text-slate-500 text-sm mt-1">Masukkan alamat email terdaftar Anda untuk menerima link pemulihan akun</p>
                </div>
                ...
                @if (session('status'))
                    <div class="mb-5 p-4 rounded-xl bg-emerald-50 border border-emerald-150 text-emerald-650 text-sm font-semibold">
                        {{ session('status') }}
                    </div>
                @endif
